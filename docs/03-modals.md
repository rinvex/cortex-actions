...
```php
use Cortex\Actions\Action;
use Cortex\Forms\Components\MarkdownEditor;
use Cortex\Forms\Components\TextInput;
use Cortex\Forms\Components\Toggle;
use Cortex\Schemas\Components\Wizard\Step;
...
use Cortex\Auth\Models\User;
use Cortex\Actions\Action;
use Cortex\Forms\Components\Textarea;
use Cortex\Forms\Components\TextInput;
...
Action::make('advance')
    ...
    ->modalContent(view('cortex.pages.actions.advance'))
```
...
Action::make('advance')
    ->action(fn (Post $record) => $record->advance())
    ->modalContentFooter(view('cortex.pages.actions.advance'))
```
...
```php
use Cortex\Support\View\Components\ModalComponent;
...
When you use database queries or other heavy operations inside modal configuration methods like `modalHeading()`, they can be executed more than once. This is because Cortex uses these methods to decide whether to render the modal or not, and also to render the modal's content.

To skip the check that Cortex does to decide whether to render the modal, you can use the `modal()` method, which will inform Cortex that the modal exists for this action and it does not need to check again:
...
Action::make('create')
    ...
    ->modalContent(view('cortex.pages.actions.create'))
```
