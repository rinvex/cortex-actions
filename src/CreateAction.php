...
class CreateAction extends Action
{
    ...
    protected function setUp(): void
    {
        ...
        $this->label(fn (): string => __('cortex.actions::create.single.label', ['label' => $this->getModelLabel()]));

        $this->modalHeading(fn (): string => __('cortex.actions::create.single.modal.heading', ['label' => $this->getTitleCaseModelLabel()]));

        $this->modalSubmitActionLabel(__('cortex.actions::create.single.modal.actions.create.label'));
        ...
        $this->extraModalFooterActions(function (): array {
            return $this->canCreateAnother() ? [
                $this->makeModalSubmitAction('createAnother', arguments: ['another' => true])
                    ->label(__('cortex.actions::create.single.modal.actions.create_another.label')),
            ] : [];
        });

        $this->successNotificationTitle(__('cortex.actions::create.single.notifications.created.title'));

        $this->groupedIcon(CortexIcon::resolve('actions::create-action.grouped') ?? Heroicon::Plus);
