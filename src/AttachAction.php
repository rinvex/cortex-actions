...
use Cortex\Support\Services\RelationshipJoiner;
use Cortex\Actions\Concerns\CanCustomizeProcess;
use Illuminate\Database\Eloquent\Relations\Relation;
use Illuminate\Database\Eloquent\Relations\BelongsToMany;

use function Cortex\Support\generate_search_term_expression;
use function Cortex\Support\generate_search_column_expression;
...
class AttachAction extends Action
{
    ...
    protected function setUp(): void
    {
        ...
        $this->label(__('cortex.actions::attach.single.label'));

        $this->modalHeading(fn (): string => __('cortex.actions::attach.single.modal.heading', ['label' => $this->getTitleCaseModelLabel()]));

        $this->modalSubmitActionLabel(__('cortex.actions::attach.single.modal.actions.attach.label'));
        ...
        $this->extraModalFooterActions(function (): array {
            return $this->canAttachAnother() ? [
                $this->makeModalSubmitAction('attachAnother', ['another' => true])
                    ->label(__('cortex.actions::attach.single.modal.actions.attach_another.label')),
            ] : [];
        });

        $this->successNotificationTitle(__('cortex.actions::attach.single.notifications.attached.title'));
    ...
    public function getRecordSelect(): Select
    {
        ...
        $select = Select::make('recordId')
            ->label(__('cortex.actions::attach.single.modal.fields.record_id.label'))
            ->required()
