...
class AssociateAction extends Action
{
    ...
    protected function setUp(): void
    {
        ...
        $this->label(__('cortex.actions::associate.single.label'));

        $this->modalHeading(fn (): string => __('cortex.actions::associate.single.modal.heading', ['label' => $this->getTitleCaseModelLabel()]));

        $this->modalSubmitActionLabel(__('cortex.actions::associate.single.modal.actions.associate.label'));
        ...
        $this->extraModalFooterActions(function (): array {
            return $this->canAssociateAnother ? [
                $this->makeModalSubmitAction('associateAnother', arguments: ['another' => true])
                    ->label(__('cortex.actions::associate.single.modal.actions.associate_another.label')),
            ] : [];
        });

        $this->successNotificationTitle(__('cortex.actions::associate.single.notifications.associated.title'));
    ...
    public function getRecordSelect(): Select
    {
        ...
        $select = Select::make('recordId')
            ->label(__('cortex.actions::associate.single.modal.fields.record_id.label'))
            ->required()
