...
Cortex includes a prebuilt action that is able to export rows to a CSV or XLSX file. When the trigger button is clicked, a modal asks for the columns that they want to export, and what they should be labeled. This feature uses [job batches](https://laravel.com/docs/queues#job-batching) and [database notifications](../../notifications/database-notifications#overview), so you need to publish those migrations from Laravel. Also, you need to publish the migrations for tables that Cortex uses to store information about exports:
...
```php
use Cortex\Custom\Exports\ProductExporter;
use Cortex\Actions\ExportAction;
...
Or if you want to add it as a table bulk action, so that the user can choose which rows to export, they can use `Cortex\Actions\ExportBulkAction`:

```php
use Cortex\Custom\Exports\ProductExporter;
use Cortex\Actions\ExportBulkAction;
use Cortex\Tables\Table;
...
The ["exporter" class needs to be created](#creating-an-exporter) to tell Cortex how to export each row.

## Creating an exporter

To create an exporter class for a model, you may use the `make:cortex-exporter` command, passing the name of a model:

```bash
php artisan make:cortex-exporter Product
```

This will create a new class in the `app/cortex/custom/src/Exports` directory. You now need to define the [columns](#defining-exporter-columns) that can be exported.

### Automatically generating exporter columns

If you'd like to save time, Cortex can automatically generate the [columns](#defining-exporter-columns) for you, based on your model's database columns, using `--generate`:

```bash
php artisan make:cortex-exporter Product --generate
```
...
```php
use Cortex\Actions\Exports\ExportColumn;
...
```php
use Cortex\Custom\Models\Order;
use Cortex\Actions\Exports\ExportColumn;
...
Cortex provides several methods for aggregating a relationship field, including `avg()`, `max()`, `min()` and `sum()`. For instance, if you wish to show the average of a field on all related records in a column, you may use the `avg()` method:
...
By default, exported files will be uploaded to the storage disk defined in the [configuration file](../../installation#publishing-configuration), which is `public` by default. You can set the `CORTEX_FILESYSTEM_DISK` environment variable to change this.

While using the `public` disk a good default for many parts of Cortex, using it for exports would result in exported files being stored in a public location. As such, if the default filesystem disk is `public` and a `local` disk exists in your `config/filesystems.php`, Cortex will use the `local` disk for exports instead. If you override the disk to be `public` for an `ExportAction` or inside an exporter class, Cortex will use that.
...
Export files that are created are the developer's responsibility to delete if they wish. Cortex does not delete these files in case the exports need to be downloaded again at a later date.
...
Cortex will chunk the CSV, and process each chunk in a different queued job. By default, chunks are 100 rows at a time. You can change this by calling the `chunkSize()` method on the action:
...
The default job for processing exports is `Cortex\Actions\Exports\Jobs\PrepareCsvExport`. If you want to extend this class and override any of its methods, you may replace the original class in the `register()` method of a service provider:

```php
use Cortex\Custom\Jobs\PrepareCsvExport;
use Cortex\Actions\Exports\Jobs\PrepareCsvExport as BasePrepareCsvExport;
...
```php
use Cortex\Custom\Policies\ExportPolicy;
use Cortex\Actions\Exports\Models\Export;
