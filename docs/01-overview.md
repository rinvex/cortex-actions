...
In Cortex, actions also handle "doing" something in your app. However, they are a bit different from traditional actions. They are designed to be used in the context of a user interface. For instance, you might have a button to delete a client record, which opens a modal to confirm your decision. When the user clicks the "Delete" button in the modal, the client is deleted. This whole workflow is an "action".
...
```php
use Cortex\Actions\Action;
use Cortex\Forms\Components\RichEditor;
...
These actions use the `Cortex\Actions\Action` class. They can open a modal if you choose, or even just a URL.
...
These actions use the `Cortex\Actions\BulkAction` class. They can open modals if you choose.
...
Infolist components can contain actions. These use the `Cortex\Infolists\Components\Actions\Action` class. They can open a modal if you choose, or even just a URL.
...
These actions use the `Cortex\Actions\Action` class. They aren't able to open modals, but they can open a URL or dispatch a Livewire event.
...
Cortex includes several prebuilt actions that you can add to your app. Their aim is to simplify the most common Eloquent-related actions:
