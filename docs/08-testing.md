...
it('links to the correct Cortex sites', function () {
    $invoice = Invoice::factory()->create();

    livewire(EditInvoice::class, [
        'invoice' => $invoice,
    ])
        ->assertActionHasUrl('Cortex', 'https://cortex.dev/')
        ->assertActionDoesNotHaveUrl('Cortex', 'https://github.com/rinvex/cortex')
        ->assertActionShouldOpenUrlInNewTab('cortex')
