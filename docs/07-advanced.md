...
These injected utilities require specific parameter names to be used. Otherwise, Cortex doesn't know what to inject.
...
Action::make('delete')
    ->rateLimit(config('cortex.auth.throttle.action.max_attempts'))
```
...
If you wish to customize the rate limit behaviour, you can use Laravel's [rate limiting](https://laravel.com/docs/rate-limiting#basic-usage) features and Cortex's [flash notifications](../notifications/sending-notifications) together in the action.
