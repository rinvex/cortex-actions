...
use Cortex\Schemas\Components\Split;
...
use Cortex\Actions\Exports\Jobs\CreateXlsxFile;
use Cortex\Actions\Exports\Jobs\ExportCompletion;
use Cortex\Actions\Exports\Jobs\PrepareCsvExport;
...
trait CanExportRecords
{
    ...
    protected function setUp(): void
    {
        ...
        $this->label(fn (ExportAction | ExportBulkAction $action): string => __('cortex.actions::export.label', ['label' => $action->getPluralModelLabel()]));

        $this->modalHeading(fn (ExportAction | ExportBulkAction $action): string => __('cortex.actions::export.modal.heading', ['label' => $action->getTitleCasePluralModelLabel()]));

        $this->modalSubmitActionLabel(__('cortex.actions::export.modal.actions.export.label'));

        $this->groupedIcon(CortexIcon::resolve('actions::export-action.grouped') ?? Heroicon::ArrowDownTray);

        $this->form(fn (ExportAction | ExportBulkAction $action): array => [
            ...($action->hasColumnMapping() ? [Fieldset::make(__('cortex.actions::export.modal.form.columns.label'))
                ->columns(1)
                ->inlineLabel()
                    ...
                    return array_map(
                        fn (ExportColumn $column): Split => Split::make([
                            Checkbox::make('isEnabled')
                                ->label(__('cortex.actions::export.modal.form.columns.form.is_enabled.label', ['column' => $column->getName()]))
                                ->hiddenLabel()
                                ->default($column->isEnabledByDefault())
                                ->live()
                                ->grow(false),
                            TextInput::make('label')
                                ->label(__('cortex.actions::export.modal.form.columns.form.label.label', ['column' => $column->getName()]))
                                ->hiddenLabel()
        ...
        $this->action(function (ExportAction | ExportBulkAction $action, array $data, Component $livewire): void {
            ...
            if ($maxRows < $totalRows) {
                Notification::make()
                    ->title(__('cortex.actions::export.notifications.max_rows.title'))
                    ->body(trans_choice('cortex.actions::export.notifications.max_rows.body', $maxRows, [
            ...
            if (
                ...
                Notification::make()
                    ...
                    ->body(trans_choice('cortex.actions::export.notifications.started.body', $export->total_rows, [
        ...
        $this->successNotificationTitle(__('cortex.actions::export.notifications.started.title'));
