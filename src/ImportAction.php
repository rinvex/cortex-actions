...
use Cortex\Panels\Facades\Cortex;
use Cortex\Support\ChunkIterator;
use Cortex\Support\Icons\Heroicon;
use League\Csv\Reader as CsvReader;
use Cortex\Forms\Components\Select;
...
use Cortex\Forms\Components\FileUpload;
use Cortex\Schemas\Components\Fieldset;
use Illuminate\Support\Facades\Storage;
use Cortex\Actions\Imports\ImportColumn;
use Cortex\Actions\Imports\Models\Import;
use Illuminate\Filesystem\AwsS3V3Adapter;
use Cortex\Actions\Imports\Jobs\ImportCsv;
use Illuminate\Contracts\Support\Htmlable;
use Cortex\Schemas\Components\Utilities\Get;
use Cortex\Schemas\Components\Utilities\Set;
use Illuminate\Contracts\Auth\Authenticatable;
use Illuminate\Validation\ValidationException;
use Cortex\Actions\Imports\Events\ImportStarted;
use Cortex\Actions\Imports\Events\ImportCompleted;
...
class ImportAction extends Action
{
    ...
    protected function setUp(): void
    {
        ...
        $this->label(fn (ImportAction $action): string => __('cortex.actions::import.label', ['label' => $action->getPluralModelLabel()]));

        $this->modalHeading(fn (ImportAction $action): string => __('cortex.actions::import.modal.heading', ['label' => $action->getTitleCasePluralModelLabel()]));

        $this->modalDescription(fn (ImportAction $action): Htmlable => $action->getModalAction('downloadExample'));

        $this->modalSubmitActionLabel(__('cortex.actions::import.modal.actions.import.label'));

        $this->groupedIcon(CortexIcon::resolve('actions::import-action.grouped') ?? Heroicon::ArrowUpTray);

        $this->form(fn (ImportAction $action): array => array_merge([
            FileUpload::make('file')
                ->label(__('cortex.actions::import.modal.form.file.label'))
                ->placeholder(__('cortex.actions::import.modal.form.file.placeholder'))
                ...
                ->required()
                ->hiddenLabel(),
            Fieldset::make(__('cortex.actions::import.modal.form.columns.label'))
                ->columns(1)
                ->inlineLabel()
        ...
        $this->action(function (ImportAction $action, array $data): void {
            ...
            if ($maxRows < $totalRows) {
                Notification::make()
                    ->title(__('cortex.actions::import.notifications.max_rows.title'))
                    ->body(trans_choice('cortex.actions::import.notifications.max_rows.body', $maxRows, [
                        ...
                        ->when(
                            ...
                            fn (Notification $notification) => $notification->actions([
                                Action::make('downloadFailedRowsCsv')
                                    ->label(trans_choice('cortex.actions::import.notifications.completed.actions.download_failed_rows_csv.label', $failedRowsCount, [
                                        'count' => Number::format($failedRowsCount),
                                    ]))
                                    ->color('danger')
                                    ->url(URL::signedRoute('cortex.imports.failed-rows.download', ['authGuard' => $authGuard, 'import' => $import], absolute: false), shouldOpenInNewTab: true)
                                    ->markAsRead(),
                            ]),
                        )
                        ->when(
            ...
            if (
                ...
                Notification::make()
                    ...
                    ->body(trans_choice('cortex.actions::import.notifications.started.body', $import->total_rows, [
        ...
        $this->registerModalActions([
            Action::make('downloadExample')
                ->label(__('cortex.actions::import.modal.actions.download_example.label'))
                ->link()
                ->action(function (): StreamedResponse {
                    ...
                    return response()->streamDownload(function () use ($csv): void {
                        ...
                        echo $csv->toString();
                    }, __('cortex.actions::import.example_csv.file_name', ['importer' => (string) str($this->getImporter())->classBasename()->kebab()]), [
        ...
        $this->successNotificationTitle(__('cortex.actions::import.notifications.started.title'));
        ...
        $fileRules = [
            ...
            File::types(['csv', 'txt'])->rules([
                function (string $attribute, mixed $value, Closure $fail): void {
                    ...
                    $fail(trans_choice('cortex.actions::import.modal.form.file.rules.duplicate_columns', count($filledDuplicateCsvColumns), [
    ...
    public function getAuthGuard(): string
    {
        ...
        if (class_exists(Cortex::class) && cortex()->isServing()) {
            return cortex()->getCurrentOrDefaultPanel()->getAuthGuard();
        }
